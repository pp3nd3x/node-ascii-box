...
const getContent = (msg, maxlength, borders, padding) => {
  ...
  return msg
    ...
    .map(line => {
      ...
      const extra = parseInt(maxlength - line.length + parseInt(bits * 5));
...
const getMaxLength = (msgArray, padding, minWidth, maxWidth) => {
  const lineLength =
    msgArray.reduce((acc, curr, index) => (curr.length > acc ? curr.length : acc), 0) + padding;
...
function box(msg, opts) {
  ...
  try {
    ...
    const padding = tableOpts.padding;
    const minWidth = tableOpts.minWidth;
    const maxWidth = tableOpts.maxWidth;

    if (tableOpts.color !== 'none') {
      borders = Object.entries(borders).reduce((acc, [key, val], index) => {
